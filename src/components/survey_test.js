...
const fetchPassage = async (userId) => {
  try {
    ...
    const res = await axios.get(`https://localhost:5004/passage?user_id=${userId}`);
    setPassage(res.data);
    resetForm();
  } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`https://localhost:5004/submit`, dataToSubmit);
      ...
      {passage ? (
        <div>
          ...
          <form onSubmit={handleSubmit} className="evaluation-form">
            ...
            {/* Passage B with conditional rendering */}
            <div className="evaluation-container comment-container">
              <div className="passage-box">
                <h2 className="passage-title">Please read the below passage again.</h2>
                <p className="passage-text">{passage.passage_b}</p>
              </div>
              <div className="rephrase-container">
                <h3>Please copy complex sentence or phrases and simplify in your own words.</h3>
                <textarea
                  value={comments.improvement_suggestions}
                  onChange={handleCommentChange}
                  className="textarea-input"
                  placeholder="Please simplify the complex sentence in your own words."
                  required
                />
              </div>
            </div>
